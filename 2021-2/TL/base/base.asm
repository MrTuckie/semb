...
; loop para checar a tecla e fazer o que tem que fazer (??)
L1:

        ; Aqui entra loop infinito para fazer a animação.

        ;pinta o circulo de branco
        ...
        ; logica para rebate do X
        cmp word[posicaox],628
            je inverte_X
        cmp word[posicaox],13
            je normal_X
        jmp continuaX
        ...
        continuaX:
            mov ax,word[somaX]
            add word[posicaox],ax 	;posicaox = posicaox+5

        ; fim da logica do rebate do X

        ; logica para rebate do y
        cmp word[posicaoy],468
            je inverte_Y
        cmp word[posicaoy],13
            je normal_Y
        jmp continuaY
        ...
        continuaY:
            mov ax,word[somaY]
            add word[posicaoy],ax 	;posicaoy = posicaoy+5
        ; fim da logica do rebate do y

        ; saindo do programa
        cmp byte[saidaFlag],1
            je L2
    jmp L1
...
; interrupção chamada quando uma tecla é pressionada
keyint:
        PUSH    AX
        push    bx
        push    ds
        ...
        ACELERA:
            cmp word[velocidade],10
            je fimFlag
            sub word[velocidade],10
            jmp fimFlag
        FREIA:
            cmp word[velocidade],100
            je fimFlag
            add word[velocidade],10
            jmp fimFlag
        saida:
            mov byte[saidaFlag],1
        fimFlag:

        IN      AL, kb_ctl
        OR      AL, 80h  ; break = make + 128
        OUT     kb_ctl, AL
        AND     AL, 7Fh
        OUT     kb_ctl, AL
        MOV     AL, eoi
