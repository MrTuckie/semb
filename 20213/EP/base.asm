...
segue: 
	...
	; checa se apertaram 'u'
	cmp al,'u'
		jne faz_nada 
	
	mov bx,word[raquetey1]
	; checa se chegou no limite 'u'
	cmp bx,427
		jge faz_nada 
	...
	; checa se apertaram 'd'
	cmp al,'d'
		jne continue 
	
	mov bx,word[raquetey1]
	
	; checa se chegou no limite 'd'
	cmp bx, 51
		jbe continue 
...
continue:
	...
	; checando se bateu na raquete
		...
		jmp raquete 
		faz_nada_2:
		; não bateu na raquete

	xMovement:
		; movimento horizontal
		; checando se está no limite
		cmp word[xPos],620
			jae left
		cmp word[xPos],20
			jle right
		...
		left:
			...
			passa_batido:
				; atualiza o placar e reinicia
				call msg_placar_comp
				jmp reseta

		; faz andar para a direita
		right:
...
; desenha toda a interface inicial
faz_interface:
	call borders
	call draw_raquete
    call msg_header
...
nome            db      'Lucas Viana Braun'
comp            db      'Computador'
x_str           db      'x'
placar_humano   db      48,48,48
placar_comp     db      48,48,48
